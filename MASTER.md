...
<div class="wrap">
  ...
  <div class="note" style="background:#fff4d6;border-color:#e7c200;border-left-color:#b38f00">
    <b>⚠ This dashboard is a historical snapshot (29 May 2026) and is partially superseded.</b>
    Since it was written the project added the <b>base schema migration</b> (it IS now in the repo),
    the authenticated <b>Patient Hub portal</b> and <b>Staff Console</b>, the identity-matcher,
    clinical-review workflow, tamper-evident audit chains, and the proxy scaffold (11 migrations total).
    For the current, authoritative status always use <code>COMPLIANCE.md</code> (single source of truth)
    and <code>DEPLOYMENT.md</code> for the execution runbook. The scores below remain broadly accurate
    (DTAC ≈ 44/100; engineering ≈ 90+/100) because the ceiling is unchanged — Trust sign-offs.
  </div>
  ...
  <div class="note" style="background:var(--pend-bg);border-color:#e7b9a6;border-left-color:var(--pend)">
    <b>Backend not provisioned yet (but all SQL is now in the repo).</b> There is still no Supabase project for
    this app. <b>Correction to the original note:</b> the base schema is <b>no longer assumed upstream</b> — it is
    now in the repo as <code>20260527000000_base_schema.sql</code> (<code>waitlist_entries</code>,
    <code>hospitals</code>, <code>auth.current_hospital_id()</code>, <code>sms_dispatch_jobs</code>), first of
    <b>11 migrations</b>. To go live: (1) create a Supabase project in <b>London — eu-west-2</b> (UK residency,
    §7; can't be changed later); (2) apply the 11 migrations in order and run <code>supabase/tests/verify.sql</code>
    — full steps in <code>DEPLOYMENT.md</code>. All backend SQL is <b>code-reviewed, not executed</b>.
  </div>
  ...
  <div class="meters">
    <div class="meter-card">
      ...
      <p>All <em>code</em> closeable without the Trust is done — incl. base schema, identity matching, clinical-review workflow, tamper-evident audit chains, proxy scaffold, two authenticated surfaces (portal + staff), and the deploy/verify runbooks. Not yet provisioned: no live Supabase project (all SQL code-reviewed, not executed).</p>
    </div>
    <div class="meter-card">
  ...
  <div class="next">
    <ul>
      ...
      <li><span class="num">②</span><div><b>(Done in repo) Base schema</b> — <code>20260527000000_base_schema.sql</code> (<code>waitlist_entries</code>, <code>hospitals</code>, <code>auth.current_hospital_id()</code>, <code>sms_dispatch_jobs</code> + <code>get_next_sms_batch</code>) is committed as the first of 11 migrations. No longer "assumed upstream".</div></li>
      ...
      <li><span class="num">④</span><div><b>Apply migrations + verify</b> — <code>supabase db push</code> applies all <b>11</b> in order (base schema → status-domain prereq → section-11 → retention/erasure → NHS-number → token generator → portal RLS → identity matching → clinical-review → audit hash chain → proxy scaffold), then run <code>supabase/tests/verify.sql</code>. Full annotated steps in <code>DEPLOYMENT.md</code>.</div></li>
